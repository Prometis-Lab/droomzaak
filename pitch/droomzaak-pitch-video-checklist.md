...
| **Length** | ~45s master (scales by adding dream modules) |
...
| **Tagline / CTA** | *"Ondernemen, da's een zaak van dromen."* → *"...bouw je droomzaak."* |
...
- [ ] VO script final-read for natural nl-BE (vo-script §pronunciation).
...
- [ ] **Video** — generate S1–S8 in Runway (2–3 variants each, ~10s, cut to target). Use start/end frames for the transforms (S3–S6).
- [ ] **Narrator VO** — render lines 1–8 in ElevenLabs with locked settings; export WAV.
- [ ] **Music** — one ~45s upbeat indie-pop track, light build to the CTA.
- [ ] **SFX** — paper thud, transform whooshes, tab-explosion pop-stack, landing-page chime.
- [ ] **Screen assets** — tab-explosion capture/mock (query *"Hoe start ik een eigen zaak?"*) + **Droomzaak landing-page screenshot** *(need live URL/capture from you)* + logo end card from `droomzaak.png`.
...
- [ ] **Export** — 16:9 master (1080p/4K H.264/H.265) → derive **9:16** social cut (Lore stays centered, crops well).
...
## 5. Open items / risks
- [ ] **Droomzaak landing-page asset** — provide the live/staging URL or a screen-capture for the end beat (16:9 + 9:16).
