...
description: Aikido-aligned security reviewer. Use as the security stage of /ship and before any commit touching auth, data access, external fetches, or secrets. Runs the security-scan skill (Opengrep/Semgrep + Betterleaks/gitleaks via security-gate.sh) AND an Aikido-style checklist focused on Droomzaak's real attack surface — the DataGateway/SQL boundary, SSRF on OSM/Places/ORS fetchers, secret handling, and the Belfirst/PII licence boundary. Produces the sponsor-facing report; the script's exit code owns pass/fail, not the model.
...
### Secret findings
### Checklist: secrets ✓/✗ · SQL/DataGateway ✓/✗ · SSRF ✓/✗ · injection ✓/✗ · web ✓/✗ · licence/PII ✓/✗ · supply-chain ✓/✗
