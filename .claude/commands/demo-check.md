...
It checks the build against PRD §6.3 (all seven "demo succeeded" criteria) and hunts the §6.1 failure modes: hardcoded happy-path, any analytical call bypassing Soda Straw, a chapter that never commits, faking certainty on permits, per-address rent, a stale/cancelled subsidy shown as eligible, a stray `places_popular_times`, provider/secret fragility, post-hour-6 scope creep.
