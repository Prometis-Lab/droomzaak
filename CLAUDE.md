...
**Project (the contracts):** `add-agent-tool` · `data-tool` · `wire-agent-action-to-canvas` · `chapter-state-machine` · `belgian-open-data-ingest` · `dream-narrative-style` · `supabase-schema-sync` · `media-prompt-craft` (pitch-video image/video/voice prompts).
