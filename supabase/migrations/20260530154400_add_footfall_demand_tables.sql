-- Migration: add the two Q3 score_locations tables to the droomzaak warehouse.
-- Applied 2026-05-30 via the loader DSN (folded from supabase/scripts/add_footfall_demand_tables.sql,
-- now archived). Idempotent (CREATE … IF NOT EXISTS) — safe to re-run / redeploy.
--
-- Column ORDER matches each pipelines/droomzaak/datasets/<name>/clean.py SELECT so the
-- loader's position-based INSERT … SELECT * lines up.
