...
ACADEMIC_ENTERPRISES = "snapshots/kbo/kbo-academic-enterprises/kbo_enterprises_*.parquet"
GEOCODE_ENTERPRISES = "snapshots/kbo/kbo-academic-enterprises-geocoded/kbo_enterprises_*.parquet"
GEOCODE_BUSINESS_UNITS = "snapshots/kbo/kbo-academic-business-units-geocoded/kbo_business_units_*.parquet"
...
def lonlat(x: str = "x", y: str = "y") -> tuple[str, str]:
    ...
    pt = f"ST_Transform(ST_Point({x}, {y}), 'EPSG:3812', 'EPSG:4326', always_xy := true)"
...
def load_sectors(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if not GEO_SECTORS.exists():
        raise FileNotFoundError("Run geo_admin/clean.py first (needs geo_sectors_gent.parquet)")
    con.execute(f"""
        CREATE TEMP TABLE sectors AS
        SELECT nis9_code, wijknr, ST_GeomFromWKB(geom_wkb) AS geom
